...
void CarGazeboPlugin::Load(gazebo::physics::ModelPtr model,
  ...
  // fake_car code here
  {
    ...
    fl_str_joint = get_joint("joint_front_left_wheel_dir");

    jc->SetPositionPID(fl_str_joint->GetScopedName(), fl_pid);

    // front right str
    fr_pid = gazebo::common::PID(1, 0, 0);
    fr_str_joint = get_joint("joint_front_right_wheel_dir");
    jc->SetPositionPID(fr_str_joint->GetScopedName(), fr_pid);

    fl_axle_joint = get_joint("joint_front_left_wheel");
    fr_axle_joint = get_joint("joint_front_right_wheel");

    // back left speed
    bl_pid = gazebo::common::PID(0.1, 0.01, 0.0);
    bl_axle_joint = get_joint("joint_back_left_wheel");
    jc->SetVelocityPID(bl_axle_joint->GetScopedName(), bl_pid);

    br_pid = gazebo::common::PID(0.1, 0.01, 0.0);
    br_axle_joint = get_joint("joint_back_right_wheel");

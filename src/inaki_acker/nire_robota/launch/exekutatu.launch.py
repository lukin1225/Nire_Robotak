...
def generate_launch_description():
    ...
    pkg_inaki_car = get_package_share_directory('inaki_acker')
    ...
    bringup_dir = get_package_share_directory('inaki_acker')
    launch_dir = os.path.join(bringup_dir, 'launch')
    world_config = DeclareLaunchArgument(
          'world',
          default_value=[os.path.join(pkg_inaki_car, 'worlds', 'etxea.world'), ''],
    ...
    rviz_config_path = os.path.join(pkg_inaki_car, 'rviz', 'nav2_default_view.rviz')
    ...
    nav2_params_path = os.path.join(get_package_share_directory('inaki_acker'), 'config/', 'nav2_params.yaml')

    behavior_tree_path = os.path.join(get_package_share_directory('inaki_acker'), 'behavior_trees', 'navigate_w_replanning_time2.xml')
    ...
    car = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_inaki_car, 'launch', 'spawn_car.launch.py'),
        )
    )
    ...
    teleop_twist = Node(
            package='teleop_twist_joy', executable='teleop_node', output='screen', parameters=[os.path.join(pkg_inaki_car, "config", "teleop_twist_joy.yaml")])
